...
python pick_seq_list.py -f groups.asm.fasta -l linkage_groups.list -o linkage_groups_only.fasta
python seqlength.py -f linkage_groups_only.fasta > linkage_groups_length.txt
```
Generate the chromatin contact matrix to evaluate genome scaffolding. Parameter `500k` here is the bin size. For our genome size equals to 400M, this is a reasonable settings. For larger genome size, bin size can be larger.
```bash
#!/bin/sh
#$ -V
#$ -cwd
...
ALLHiC_plot SN_HiC.bam groups.agp linkage_groups_length.txt 500k pdf
```
Output of this step are `.pdf` files of plotting results.
